...
import { timeTracker } from './time-tracker.js'; // Assuming timeTracker is defined in time-tracker.js
...
function renderTasks() {
    ...
    currentUserTasks.forEach(task => {
        ...
        taskElement.innerHTML = `
            <div class="task-content">
                ...
                <div class="task-time-tracking">
                    <span class="task-time-spent">${timeTracker.formatTime(task.timeSpent)}</span>
                </div>
            </div>
        ...
        startTimerBtn.addEventListener('click', () => {
            if (taskTimer === null) {
                taskTimer = timeTracker.startTaskTimer(task.id);
                startTimerBtn.innerHTML = '<i class="fas fa-pause"></i>';
            } else {
                const timeSpent = timeTracker.stopTaskTimer(task.id);
        ...
        taskElement.querySelector('.task-actions').prepend(startTimerBtn);
    });
...
function saveTask(event) {
    ...
    if (taskId) {
        ...
        const newTask = {
            ...
            timeSpent: 0,
            timeLimit: config.timeTracking.defaultTaskTimeLimit
        };
